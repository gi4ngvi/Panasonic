...
public class TH_TH_RegistrationTest extends SeleniumTest {
    CustomerInformation cus, cusMissFirstName, cusMissLastName, cusMissNickName, cusMissEmail, cusMissPass,
            cusMissConfirmPass, cusWithPassAndConfirmPassNotMatch, cusEmpty, cusAlready, cusInvalid;
    ...
    protected void preCondition() {
        ...
        cusEmpty = new CustomerInformation.Builder().email("").password("").passwordConfirm("").firstName("").lastName("").nickName("").identityCardNumber("").dateOfBirth("").typeOfIdentityCard("-- Choose --").build();
    ...
    public void registerWithInfoValid() {
        ...
        ReportUtility.getInstance().logInfo(cus.getEmail());
        ReportUtility.getInstance().logInfo(cus.getPassword());
        ReportUtility.getInstance().log(LogStatus.WARNING, "Temporary accept term and condition to workaround for ticket PANAMAINTE-332. Waiting to update testcase!");
        registerPage.setAcceptTermAndConditions(true);
