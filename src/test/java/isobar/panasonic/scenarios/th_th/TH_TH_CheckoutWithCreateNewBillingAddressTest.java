...
public class TH_TH_CheckoutWithCreateNewBillingAddressTest extends SeleniumTest {
    ...
    protected void preCondition() {
        ...
        billingAddressEmpty = new Address.Builder().firstName("").lastName("").company("").telephone("").zip("").street("").state("กรุณาเลือกจังหวัด").build();
        cus.setShippingAddress(shippingAddress);
        cus.setBillingAddress(billingAddress);
        shippingMethod = ShippingMethod.SelfCollection;
        MSG_ORDER_SUCCESS = "CLUB Panasonic ขอขอบคุณสำหรับการแลกของรางวัลค่ะ";
        MSG_REQUIED = "นี่คือข้อมูลที่จำเป็น.";
    }
